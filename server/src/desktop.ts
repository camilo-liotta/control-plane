 ...
 * entrada de la web (useInboxCount): sesiones que te necesitan, propuestas listas para enviar y
 * tareas abiertas que frenan a una sesión.
 */
export function desktopSummary({ db, sessions }: SummaryDeps): DesktopSummary {
  const all = sessions.list()
  const ready = db.listDrafts({ states: ["ready"] })
  const blocking = db.listTasks().filter((t) => t.status === "open" && t.blocking)
  const projects = db.listProjects().map((p) => {
    ...
    return {
      ...
      name: p.name,
      needs:
        own.filter((s) => s.status === "needs_input").length +
        ready.filter((d) => d.projectId === p.id).length +
        blocking.filter((t) => t.projectId === p.id).length,
      working: own.filter(isWorking).length,
    }
  })
  return {
    needs: all.filter((s) => s.status === "needs_input").length + ready.length + blocking.length,
