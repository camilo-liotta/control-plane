...
// La carpeta se borra y se regenera: solo si no existe, está vacía o es un bundle anterior.
...
import { BUNDLE_MARKER, checkOut } from "./out-guard.ts"
...
// La carpeta se borra entera: antes, que no sea algo que no hay que borrar.
const refused = checkOut(out, { serverDir })
if (refused) {
...
fs.writeFileSync(path.join(out, BUNDLE_MARKER), "Bundle de control-plane: npm run bundle -w server la borra y la regenera.\n")
